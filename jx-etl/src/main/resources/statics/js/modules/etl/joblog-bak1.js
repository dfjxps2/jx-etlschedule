		...
		loadsys: function(componentId){
			...
			$.ajax({
                ...
                success: function(data){
                    if(data.code == 0){
						// alert(JSON.stringify(data.allsys));
                        var len = data.allsys.length;
                        var syses = data.allsys;
                        var optionString = "<option selected value=''>" + "全部" + "</option>";
                        for (var i = 0; i < len; i++) {
                            optionString += "<option value=\'"+ syses[i] +"\'>" + syses[i] + "</option>";
                            // alert("i==" + optionString);
                        }
                        // alert(optionString);
                        var myobj = document.getElementById(componentId);
                        // if (myobj.options.length == 0)
                        // {
                            $("#" + componentId).html(optionString);
